    ...
    mpReplaced(static_cast<MapPoint*>(NULL)), mfMinDistance(0), mfMaxDistance(0), mpMap(pMap)
{
    ...
    mNormalVector = cv::Mat::zeros(3,1,CV_32F);  //列向量

    /// MapPoints can be created from Tracking and Local Mapping. This mutex avoid conflicts with id.
    //锁了之后，是不是构造函数同一时刻只能被一个线程访问，这样id就不会乱！
...
{
    unique_lock<mutex> lock(mMutexFeatures);
    if(mObservations.count(pKF))  //mObservation是map类型，count返回被查找元素的个数，有的话返回1
        return;
    // 记录下能观测到该MapPoint的KF和该MapPoint在KF中的索引
    mObservations[pKF]=idx;  //如果没有，插入元素pKF并修改属性
...
void MapPoint::ComputeDistinctiveDescriptors()
{
    ...
    {
        unique_lock<mutex> lock1(mMutexFeatures);  //TODO lock1改为lock可以吗？
        if(mbBad)
            return;
        observations=mObservations;
    }  ///单单一个语句块，难道是说锁定的部分只有这一部分？
	...
	Distances.resize(N, vector<float>(N, 0));  //设定维度
    ...
    {
        unique_lock<mutex> lock(mMutexFeatures); ///对锁的概念还是很迷 
